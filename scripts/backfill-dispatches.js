...
async function main() {
  ...
  for (const file of logFiles.sort()) {
    ...
    for (const line of raw.split('\n')) {
      ...
      const match = (obj.msg || '').match(PROMPT_RE);
      if (!match) continue;

      const body = JSON.stringify({ ts: obj.ts, member: obj.mem, prompt: match[2].trim() });
      try {
        ...
        if (res.ok) {
          posted++;
          process.stdout.write(`✓ ${obj.mem} @ ${obj.ts.slice(0, 19)}: ${match[2].slice(0, 60)}\n`);
        } else {
          failed++;
          process.stdout.write(`✗ HTTP ${res.status} for ${obj.mem}\n`);
        }
      } catch (e) {
        failed++;
